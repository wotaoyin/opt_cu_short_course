...
void worker(int transpose,double* matrix,int m, int n, double* v_in, double* v_out, int start, int block_size){
  if(transpose == 0){
    for( int i=0; i< block_size; ++i ){
      dot( &v_out[i+start], matrix+(start+i)*n, 1, v_in,1,n);
    }
  }
  else{
    for( int i=0; i< block_size; ++i ){
      dot( &v_out[i+start], matrix+i+start, n, v_in,1,m);
    }
  }
} 
...
int main(){
  ...
  for(size_t i=0; i < nthreads-1; ++i){
    threads.push_back( std::thread(worker, 0, matrix, m, n,  x,  y, block_start, block_size  )  );
    block_start += block_size;
  }
  threads.push_back( std::thread(worker, 0, matrix, m, n,  x,  y, block_start, remainder_block)  );
  ...
  block_start = 0;
  for(size_t i=0; i < nthreads-1; ++i){
    threads.push_back( std::thread(worker, 1, matrix, m, n,  y,  x, block_start, block_size_transpose  )  );
    block_start += block_size_transpose;
  }
  threads.push_back( std::thread(worker, 1, matrix, m, n,  y,  x, block_start, remainder_block_transpose)  );
